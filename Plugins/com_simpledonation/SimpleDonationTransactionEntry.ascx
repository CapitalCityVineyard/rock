...
<script type="text/javascript">
    ...
    function sumTotals() {
        ...
        $('.account-amount').find('.form-control').each(function () {
    ...
    function calculateTotal() {
        ...
        var total = coverFeesCheckbox.is(':checked') ? amount + offsetAmount : amount;
    ...
    function initializeCoverFees() {
        var coverFeesCheckbox = $('#payment-container').find('.cover-fees');
        var formControls = $('.account-amount').find('.form-control');

        var tooltip = '&nbsp;<i class="fa fa-info-circle" data-toggle="tooltip" title="" data-original-title="Every donation has a transaction fee deducted - by checking this option you&apos;ll cover that fee and <%= _organizationName %> will get 100% of your original donation amount."></i>';
        ...
        coverFeesCheckbox.on('change', calculateTotal);
        $('input[id*="rblSavedAccount"]').on('change', calculateTotal);
        formControls.on('input', calculateTotal).on('blur', calculateTotal);
        calculateTotal();
    }
    ...
    Sys.WebForms.PageRequestManager.getInstance().add_endRequest(initializeCoverFees);
</script>