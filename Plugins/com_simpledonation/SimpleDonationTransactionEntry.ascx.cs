...
TransactionAccountDetails: [
  ...
  },
  {
    ...
    [BooleanField( "Cover Fees Checked", "Determines whether the 'cover fees' checkbox is checked by default", false, "", 34 )]
...
",
        ...
        private void RegisterFeeRates()
        {
            const string feeRatesKey = "com.SimpleDonation.FeeRates";
            FeeRates feeRates = ( FeeRates ) Cache.Get( feeRatesKey );

            if ( feeRates == null )
            {
                feeRates = _feeCalculationService.GetFeeRates();
                Cache.Insert( feeRatesKey, feeRates, null, System.Web.Caching.Cache.NoAbsoluteExpiration, TimeSpan.FromMinutes( 10 ) );
            }

            hfAchRate.Value = feeRates.AchRate.ToString();
            hfCardRate.Value = feeRates.CardRate.ToString();
            hfCapAch.Value = feeRates.CapAch.ToString();
        }
